...
class Node:
    def __init__(self, now_board, parent=None, action=None, color="", ucb_strategy="ucb1"):
        self.visits = 0
        self.reward = 0.0
        ...
        self.action = action
        self.color = color
        self.ucb_strategy = ucb_strategy  # UCB策略（"ucb1" 或 "ucb2"）
    ...
    def get_ucb(self, ucb_param):
        ...
        if self.ucb_strategy == "ucb1":
            ...
            # UCB2公式
            explore = math.sqrt(2.0 * math.log(1 + self.parent.visits) / (1 + float(self.visits)))
...
class AIPlayer:
    def __init__(self, color, ucb_strategy="ucb1"):
        self.max_times = 150
    ...
    def select(self, node):
        ...
        if len(node.children) == 0:   # 叶子，需要扩展
            return node
        if node.full_expanded():    # 完全扩展,递归选择ucb最大的子节点
            max_node = None
            max_ucb = -sys.maxsize
    ...
    def extend(self, node):
        if node.visits == 0:    # 自身还没有被访问过，不扩展，直接模拟
            ...
            if node.color == 'X':
                ...
                new_color = 'X'
            for action in list(node.now_board.get_legal_actions(node.color)):  # 把所有可行节点加入子节点列表，并初始化
